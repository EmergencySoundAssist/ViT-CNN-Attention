...
핵심 통찰: **사이렌의 정지 기준 주파수·사이클을 데이터로 실측**했고, 도플러로 속도가 어떻게 변하는지를 **물리적으로 검증**했습니다. 절대 속도는 단일 관측에서 식별 한계(±56 km/h)가 있지만, **통과(pass-by) 자기참조**로 정밀해지고(5–8 km/h), 무엇보다 **"빠름/느림 단계"** 판별엔 충분합니다.
...
| 파라미터 | **0.110M** (7.6× 작음 — 비대칭 자체가 비교축) | 0.838M |
