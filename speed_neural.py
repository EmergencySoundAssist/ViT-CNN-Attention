...
def f0_target(w_clean):
    """깨끗한 합성 윈도우 → Viterbi f0 → 중앙값제거 log, 길이 L로 리샘플 (학습 타깃)."""
    _, f0 = ds.viterbi_f0(w_clean.astype(np.float64), SR)
    lf = np.log(np.clip(np.nan_to_num(f0, nan=1.0), 1.0, None))
    lf = lf - np.median(lf)
    return np.interp(np.linspace(0, 1, L), np.linspace(0, 1, len(lf)), lf).astype(np.float32)


def mel_of(w):
    m = D.logmel(w.astype(np.float32))
    if m.shape[1] < L:
        m = np.pad(m, ((0, 0), (0, L - m.shape[1])), constant_values=np.float32(np.log(D.LOG_EPS)))
    m = m[:, :L]
    return ((m - m.mean()) / (m.std() + 1e-5)).astype(np.float32)


def gen(pool, n, seed, snr=None, v=None, d=None, keep_w=False):
    """노이즈 멜(입력) + 깨끗한 f0(보조타깃) + v(주타깃) [+ 윈도우(물리 비교용)]."""
    ...
    for _ in range(n):
        ...
        M.append(mel_of(w)); F.append(f0_target(w_clean)); Y.append(np.float32(vv))
        if keep_w:
            W.append(w)
...
class NeuralSpeed(nn.Module):
    ...
    def __init__(self, C=64, hidden=64):
        super().__init__()
        self.enc = nn.Sequential(
            ...
            nn.AdaptiveAvgPool2d((1, L)),                 # 주파수만 1로, 시간 L 보존
        )
...
def main():
    ...
    t0 = time.time()
    M, F, Y, _ = gen(tr_pool, args.train, args.seed)
    print(f"  완료 ({time.time()-t0:.0f}s) 멜{M.shape} f0{F.shape}")

    model = NeuralSpeed().to(device)
    ...
    for ss in snrs:
        for d in d_mins:
            ...
            for v in speeds:
                Mx, Fx, _, W = gen(te_pool, len(te_pool), args.seed + 700 + int(v), snr=ss, v=v, d=d, keep_w=True)
                with torch.no_grad():
