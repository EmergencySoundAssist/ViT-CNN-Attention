...
| 출력 | 라벨 출처 | 누가 잘하나 |
|------|----------|-----------|
| siren/horn/noise **검출** | 진짜 데이터 28,643개 | **모델**(ViT/CNN+Attn) |
| 접근/이탈 **방향**, **속도** | 진짜 라벨 없음 → 합성만 | **물리 DSP** (sim-to-real 갭 0) |

데이터에 방향·속도 정답이 없으므로, 그걸 학습으로만 풀면 **합성→실제 전이 도박**이 된다. 물리 DSP는 학습이 없어 그 도박이 없고, 해석가능하며(“주파수가 960→890으로 떨어짐 → 이탈”), 거의 공짜다.
...
- **라벨 자동 생성**: 합성 파라미터 (v, d_min, SNR, t₀)를 우리가 정하므로 (스펙트로그램 → 속도 tier · 방향 · 통과시각) 학습쌍이 무한 생성됨.
- 랜덤화 권장 범위: v ∈ 0~80 km/h (0 포함 — "정지" tier), d_min 5~30 m, SNR 5~20 dB, 시작 위상 랜덤.
...
**B: DL 속도 head (구축 예정)**
- 입력: 검출 backbone의 feature 공유(추가 비용 ~0) 또는 별도 멜. 출력: {정지, 접근-느림, 접근-빠름} tier + 접근/이탈 방향 logit (멀티태스크: tier CE + 방향 BCE).
...
2. **지표**: 속도 tier 정확도·혼동행렬, 방향(접근/이탈) 정확도, 참고용 km/h MAE.
...
절대 km/h 대신 단계로 단순화 (경계는 나중 캘리브레이션):
...
→ 두 방식 모두 **3-tier 분류 문제**로 환원되어 절대정밀도 부담이 사라짐.
