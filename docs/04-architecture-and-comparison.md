...
- 실측: 합성 pass-by에서 **중앙값 오차 5–8 km/h, 10 dB 잡음까지 유지** (12 s·d=8 m 조건 — 원거리 차선 기하 바이어스는 [docs/03](03-doppler-speed.md) §기하 한계). 평균 오차 ~14 km/h는 소방차 극단 사이클 outlier 때문이며 중앙값 필터가 흡수.
...
- 단 ① 마이크 ≥2 어레이 하드웨어 필요(선정 과제) ② bearing도 잡음·잔향에 취약하므로
  도플러 접근/이탈 부호는 **독립 교차검증**으로 유지 ③ 방식 B(DL)의 입력은 단일채널 유지.

파이프라인: 검출 → TDOA 방향각 θ(t) → 기하 보정된 도플러 속도 → 교차검증·융합.
...
② 2-mic은 전후 모호(좌우만) → **4-mic 권장** ③ TDOA 경로는 어레이 원샘플레이트(48 kHz) 유지
