...
**정보 한계 주의**: 증강을 아무리 미세하게 해도 물리적 정보량은 못 넘는다 — 통과 전이 관측 시 ~5–8 km/h, 정상상태 단일 윈도우의 절대속도는 ±56 km/h 바닥 ([docs/03](03-doppler-speed.md)). 연속 회귀는 이 한계 *안에서* 해상도를 최대로 쓰는 설계다.
...
- 균일 배속을 라벨로 쓰면 source 피치와 혼동 → **라벨 모순** → 학습 불가 (실측 ±56 km/h 바닥)
...
| 2 | 피치 시프트 | ±2 semitone | Salamon & Bello 2017 | 실측 개체 산포 **피치 CV 12–22%** 커버 — 같은 차종도 사이렌 음높이가 다름 |
| 3 | 타임 스트레치 | 0.85–1.15× (피치 유지) | Salamon & Bello 2017 | 실측 사이클 산포(CV 4.6–7.2%, 소방 이봉) 커버 |
...
| 5 | 게인 | ±6 dB | Salamon & Bello 2017 (DRC 변형) | 거리 1–80 m 산포, 마이크 감도 — **윈도우별 정규화에서 항등이라 멜 파이프라인에선 생략** |
| 6 | 배경잡음 합성 | SNR 5–20 dB, noise 클래스에서 | Salamon & Bello 2017 | 실도로 SNR. **우리 noise 17,737개를 잡음원으로 재활용** — 외부 잡음 데이터 불필요 |

**패딩 정책 (5 s 미만 클립 — horn 중앙값 2.8 s가 절반 이상 해당):**
무음(log-eps) 패딩은 실환경 5 s 버퍼에 존재하지 않는 입력이라 **train-serve 갭**을 만든다.
→ 패딩 영역을 버리지 않고 **noise 클립을 깔아 전체 윈도우를 채운다** (작은 데이터 최대 활용). 단 4가지 리스크 통제:

1. **노이즈 소스는 반드시 같은 split에서** — 다른 split 파일을 배경으로 쓰면 새로운 누수 채널.
2. **val/test는 결정적**: (배경 파일, SNR)을 청크 키 해시로 고정 — 재현 가능, seed 무관.
   학습은 epoch마다 랜덤 (배경 파일, SNR 5–20 dB).
3. **SNR은 이벤트(비패딩) 구간 전력 기준** — 전체 윈도우 전력 기준이면 짧은 경적이 과도하게 묻힘.
4. **반사/타일 패딩 금지**: 경적을 반복시키면 주기적 진폭 변조 = **사이렌 시그니처 위조** (검출 경계 오염).

부수 효과: 라벨 의미가 "윈도우 어딘가에 경적 존재"가 되어 배포 의미와 일치하지만 horn 난이도가 올라감
→ attention 풀링(Kong 2018)의 필요 근거가 강해지고, 지표에서 **horn recall을 따로 관찰**한다.

**구현 (`augment.py`)**: 멜 캐시 위에서 동작 — 파형 재계산 없이 **1.1 ms/청크**.
배속·피치·스트레치는 HTK mel 중심주파수 정확 매핑의 시간×주파수 워프(시간축은 전체-파일 멜에서
넓게 슬라이스 — 윈도우 밖 내용 보존), 잡음 합성은 파워 도메인 `P_s + g·P_n`(독립 신호에서 정확,
SNR은 이벤트 프레임 기준, same-split, 결정적 모드 지원 — 위 패딩 정책 그대로).
Mixup/CutMix는 `train.py` 배치단. 적용: `python train.py --aug {wave, wave_spec, full}`.
...
| # | 기법 | 출처/근거 | 왜 |
|---|------|----------|-----|
| 11 | **synth_passby** | retarded-time 물리 모델 (자체 구현, [docs/03](03-doppler-speed.md)에서 검증 — 사이클 오차 1.8%) | **속도 라벨을 만들 유일하게 올바른 방법** (균일 배속은 라벨 모순) |
...
- **경보 지연(onset latency)**: noise→siren 합성 스트림에서 가청 onset → 1차 경보(연속 2틱 양성, 검출 4 Hz)까지 시간 — **목표 ≤1.5 s** ([docs/04](04-architecture-and-comparison.md) §지연 예산)
...
7. (선택) **DSP 룰 검출기** (Viterbi 트랙 안정성 + 사이클 주기 게이트) vs B1 — "왜 학습 검출인가"의
   정량 근거. 트래커는 기권 없이 아무 음원에나 트랙을 내놓으므로 룰만으론 헬기·경적 hard negative에서
   무너질 것이 예상되며, 그 격차 자체가 보고서의 논거 ([docs/04](04-architecture-and-comparison.md) §의존성 정리)
